...
class ActiveProjectArguments(BaseModel):
    name: str | None = Field(default=None, description="A project name or id; the active or in-view project when omitted")
...
class ActiveProjectAction:
    ...
    definition = ToolDefinition(
        ...
        keywords=("project", "my tasks", "open tasks", "what did we decide", "current focus", "what am i working on", "unfinished"),
    )
    ...
    def execute(self, request: ActionRequest, context: object) -> ActionResult:
        ...
        source = Source("active_project", "memory", str(service.path) if service.path else None)
        if project is None:
            listing = ", ".join(str(item.get("name")) for item in service.active_projects()) or "none yet"
        ...
        results = [text_result(description, source=source, title=str(project.get("name")), format="text")]
        tasks = service.open_tasks(project)
        if tasks:
            results.append(table(("id", "task", "since"), [(item["id"], item["text"], item.get("created_at")) for item in tasks], source=source, title=f"Open tasks for {project.get('name')}"))
        decisions = service.active_decisions(project)
        if decisions:
            results.append(table(("decision", "made"), [(item["decision"], item.get("made_at")) for item in decisions], source=source, title=f"Decisions for {project.get('name')}"))
        return ActionResult(status="success", message=description, action=self.name, resolved_target=str(project.get("id")), results=tuple(results))


class ProjectUpdateArguments(BaseModel):
    change: Literal["task", "task_done", "decision", "focus"] = Field(description="task adds an open task, task_done closes one by id, decision records a decision, focus sets the current focus")
    text: str = Field(default="", description="The task, decision, or focus text; for task_done the task id")
    project: str | None = Field(default=None, description="Project name or id; the active or in-view project when omitted")
...
class ProjectUpdateAction:
    ...
    definition = ToolDefinition(
        ...
        keywords=("add a task", "remember to", "todo", "we decided", "decision", "mark done", "task done", "set the focus", "focus on"),
    )
    ...
    def validate(self, request: ActionRequest, context: object) -> ValidationResult:
        ...
        if service is None:
            return ValidationResult(ok=False, error=NO_SERVICE)
        project = service.get(str(arguments.project)) if arguments.project else _active(service, context)
        if project is None:
            return ValidationResult(ok=False, error="No project is active or in view; say which project.")
        text = " ".join(arguments.text.split())
        if arguments.change == "task_done":
            if not text.isdigit():
                return ValidationResult(ok=False, error="task_done needs the task id in text")
        elif not text:
        ...
        return ValidationResult(
            ...
            resolved_arguments={"change": arguments.change, "text": text, "project": str(project.get("id"))},
            confirmation_preview=ConfirmationPreview(summary=summary, target=str(service.path or "projects"), title="Project"),
    ...
    def execute(self, request: ActionRequest, context: object) -> ActionResult:
        ...
        if project is None:
            return ActionResult(status="failed", message="The project is gone.", action=self.name, error="no_project")
        ...
        try:
            if change == "task":
                ...
                message = f"Task {entry['id']} added to {project['name']}: {entry['text']}"
            elif change == "task_done":
                entry = service.complete_task(project, int(text))
                message = f"Closed task {entry['id']} on {project['name']}: {entry['text']}"
            ...
            return ActionResult(status="failed", message=str(error), action=self.name, error="invalid")
        return ActionResult(status="success", message=message, action=self.name, resolved_target=str(project.get("id")))


PROJECT_ACTIONS = (ActiveProjectAction, ProjectUpdateAction)

__all__ = ["ActiveProjectAction", "PROJECT_ACTIONS", "ProjectUpdateAction"]
