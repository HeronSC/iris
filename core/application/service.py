...
class IrisApplication:
    ...
    def _details_recalled_context(self, *, status: IrisStatus, response_type: str) -> str:
        if status != IrisStatus.COMPLETE or response_type not in {"text", "markdown"}:
            return ""
        recalled = getattr(self.coordinator, "_last_recalled_public_context", None)
        if not isinstance(recalled, dict):
            return ""
        return self._format_recalled_context(recalled)

    def _format_recalled_context(self, payload: dict[str, Any]) -> str:
        current = payload.get("current_topic")
        related = payload.get("related_topics")
        lines: list[str] = []

        if isinstance(current, dict):
            lines.extend(self._format_public_topic_section("Recovered topic", current, include_summary=False))

        if isinstance(related, list) and related:
            for index, item in enumerate(related, start=1):
                if not isinstance(item, dict):
                    continue
                heading = "Related topic" if index == 1 else f"Related topic {index}"
                section = self._format_public_topic_section(heading, item, include_summary=False)
                if section:
                    if lines:
                        lines.append("")
    ...
    def _format_public_topic_section(self, heading: str, topic_payload: dict[str, Any], *, include_summary: bool) -> list[str]:
        title = str(topic_payload.get("title", "")).strip()
        summary = str(topic_payload.get("summary", "")).strip()
        goal = str(topic_payload.get("goal", "")).strip()
        requirements = topic_payload.get("requirements", []) if isinstance(topic_payload.get("requirements"), list) else []
        items = topic_payload.get("items", []) if isinstance(topic_payload.get("items"), list) else []
        rejected = topic_payload.get("rejected_items", []) if isinstance(topic_payload.get("rejected_items"), list) else []
        questions = topic_payload.get("open_questions", []) if isinstance(topic_payload.get("open_questions"), list) else []
        change_history = topic_payload.get("change_history", []) if isinstance(topic_payload.get("change_history"), list) else []

        lines: list[str] = []
        if not title and not summary and not goal and not requirements and not items and not rejected and not questions and not change_history:
            return lines
        ...
        if items:
            ...
            for index, item in enumerate(items[:6], start=1):
                if not isinstance(item, dict):
                    continue
                lines.append(f"{index}. {str(item.get('name', 'Option')).strip()}")
        ...
        if rejected:
            ...
            for item in rejected[:6]:
                if not isinstance(item, dict):
                    continue
                reason = str(item.get("rejection_reason", "Rejected by user")).strip()
                lines.append(f"- {str(item.get('name', 'Option')).strip()}")
        ...
        if change_history:
            ...
            for entry in change_history[-5:]:
                if not isinstance(entry, dict):
                    continue
                lines.append(f"- {str(entry.get('message', '')).strip()}")
        return lines
